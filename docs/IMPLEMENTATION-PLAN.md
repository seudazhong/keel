...
1. **Contract-first.** Freeze `keel-core` Protocols (Tool, EventStore, ProviderGateway, PermissionEngine, PromptAssembler, AgentSpec) and the **event vocabulary + REST schema** before implementing behind them. Surfaces consume a generated SDK.
...
4. **Risk-first spikes.** The four correctness-critical mechanisms are proven in M0 with acceptance tests, not discovered in M2.
...
**Exit:** `docker compose --profile dev up` healthy; a no-op agent emits `run.started … run.ended{reason=completed}` visible over SSE; CI green; four spikes proven.
...
4. **State/memory (D):** event store + projectors (`messages`/`parts`); **core memory blocks** (+versioning); session persistence + resume.
...
9. **Extensibility (G):** skills loader (progressive disclosure), MCP client (stdio+HTTP/SSE, allow-list), `tool_search` discovery.
10. **Observability (H):** Langfuse trace→observation→score; cost/token accounting (incl. cache-read).

**Exit (KPIs):** internal **task-suite ≥ 80%**; invariant acceptance suite green; `full` profile up in one command; a new SDK tool **and** a new MCP server each added in **< 30 min**; 100% of runs traced with cost.
...
```mermaid
flowchart LR
    seams["M0 seams + spikes<br/>S1–S4"] --> loop["Loop α"]
    loop --> prov["Provider"]
...
- **Invariant gates:** the nine acceptance tests in `DESIGN-REVIEW.md §5` block merges for their milestone.
...
1. **Freeze the contracts** — event vocabulary + `keel-core` Protocol signatures + REST v0 (one review).
2. **Stand up** compose `dev` + CI skeleton (lint/type/unit/build + record-replay stub).
3. **Land spikes S1–S4** with acceptance tests.
