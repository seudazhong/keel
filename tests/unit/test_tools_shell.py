...
async def test_shell_echo(tmp_path: Path) -> None:
    result = await ShellTool(tmp_path).run({"command": "echo keeltest"}, _ctx())
    assert result.ok
...
async def test_shell_timeout(tmp_path: Path) -> None:
    ...
    result = await ShellTool(tmp_path, timeout=0.2).run({"command": slow}, _ctx())
    assert not result.ok
    assert "timed out" in result.output


async def test_shell_nonzero_exit(tmp_path: Path) -> None:
    result = await ShellTool(tmp_path).run({"command": "exit 3"}, _ctx())
    assert not result.ok
