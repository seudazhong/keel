...
async def test_repository_scope_isolation_cursor_and_delivery_replay(
    ...
    await repo_a.put_cursor(
        "fixture", binding.id, "default", "next", resource_id=resources[0].id
    )
    assert (await repo_a.get_cursor(
        "fixture", binding.id, "default", resource_id=resources[0].id
    )).value == "next"  # type: ignore[union-attr]
